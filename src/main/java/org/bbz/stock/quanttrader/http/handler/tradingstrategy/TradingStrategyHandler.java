package org.bbz.stock.quanttrader.http.handler.tradingstrategy;
 ...
 */
@Slf4j
public class TradingStrategyHandler extends AbstractHandler {

//  private final JWTAuth jwtAuthProvider;

  public TradingStrategyHandler(EventBus eventBus) {
    super(eventBus);
...
//        restAPI.route( "/login" ).handler( this::login );
  ...
  private void create(RoutingContext ctx, JsonObject userJson) {
    checkArgumentsStrict(userJson, "name", "desc","className",
        JsonConsts.MONGO_DB_ID, "owner");

    userJson.remove(JsonConsts.MONGO_DB_ID);//去掉_id，以便让mongodb自动生成

    DeliveryOptions options = new DeliveryOptions()
        .addHeader("action", EventBusCommand.DB_TRADING_STRATEGY_CREATE.name());

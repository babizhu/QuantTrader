...
//    }
     ...
     */
    @Override
    public void getSimpleKBar( String stockId, KLineType kLineType, int count, Handler<AsyncResult<List<SimpleKBar>>> resultHandler ){
//        String uri = "/?code=" + stockId + "&&ktype=" + kLineType.toStr() + "&&count=" + count;
    ...
    /**
     * 专用于getCurrentKbar的转换函数
     * getCurrentKbar方法从http得来的数据即使是数字也包含有引号，所以需要特殊处理
     *
    ...
    public void getCurrentKbar( String stockId, Handler<SimpleKBar> resultHandler ){
        ...
        final Future<SimpleKBar> future = sendRequest( uri, res -> parseResponse( res.toJsonArray().getJsonObject( 0 ) ) );
        future.setHandler( res -> {
            if( res.succeeded() ) {
